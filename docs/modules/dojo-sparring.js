...
(function() {
  ...
  function drawFractalForm(c, time, highlight) {
    ...
    // Outer glow
    var glowRadius = size * 2 * glow;
    var grad = ctx.createRadialGradient(0, 0, size * 0.3, 0, 0, glowRadius);
    grad.addColorStop(0, hslStr(hue, 80, 60, 0.15 * glow));
    grad.addColorStop(0.5, hslStr(hue, 60, 40, 0.05 * glow));
    ...
    if (highlight > 0) {
      ctx.strokeStyle = hslStr(hue, 90, 70, highlight * 0.6);
      ctx.lineWidth = 2;
      ctx.beginPath();
  ...
  function drawFractalBranch(x, y, size, depth, hue, glow, angle, style) {
    ...
    var alpha = 0.3 + glow * 0.5;
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(angle);

    // Core shape varies by style
    ctx.fillStyle = hslStr(hue, 70 + depth * 5, 40 + depth * 8, alpha);
    ctx.strokeStyle = hslStr(hue, 80, 60, alpha * 0.8);
    ctx.lineWidth = 1;
  ...
  function simulateRound(challenge, cA, cB, progress) {
    ...
    var sizeShift = advantage * 30;
    cA.targetSize = 60 + sizeShift;
    cB.targetSize = 60 - sizeShift;
    cA.targetGlow = 0.5 + clamp(advantage, 0, 0.5);
    cB.targetGlow = 0.5 + clamp(-advantage, 0, 0.5);
  ...
  function drawStarfield(time) {
    ...
    ctx.globalAlpha = 0.3;
    for (var i = 0; i < 30; i++) {
      var sx = (Math.sin(i * PHI * 100) * 0.5 + 0.5) * canvas.width;
      var sy = (Math.cos(i * PHI * 77) * 0.5 + 0.5) * canvas.height * 0.3;
  ...
  function buildUI() {
    ...
    container.innerHTML = '';
    container.style.cssText = 'position:relative;width:100%;height:100%;min-height:500px;background:#060a14;overflow:hidden;';
