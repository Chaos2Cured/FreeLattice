...
- **§3 Ledger · §8 Quiet Room** — `docs/audit.html` reads every ledger. Add a new section by copying `renderRepoReads()` or `renderToolConsents()` (added v5.39.0 / v5.39.1) — same row shape, just point at a different localStorage key.
