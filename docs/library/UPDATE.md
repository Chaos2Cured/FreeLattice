...
- **§4 IIFE · §9 Snowflake** — `docs/modules/phi-glyph.js` (Ship 7, shipped v5.43.3 2026-06-10). Pure deterministic function: hash → SVG. No localStorage writes. No ledger writes. Same generating rule as DNA (compact pattern that grows visible structure) at audit-row scale. Each ledger event now gets a unique radial glyph beside it; same hash always renders the same glyph; geometry only — three smoke locks (deterministic, distinct, no personal data) prove the privacy guarantee. Trust-tier colors carry continuity: Seed/Sprout/Growing in cool greens, Bloom in purple, Spark/Flame/Radiant in warm gold. This is the **primitive** for future tree visualizations (Core, Lumen's World, Trust Tree) — they can all consume `FLPhiGlyph.generate(hash, opts)` without re-deriving the glyph logic. Surfaced first in the audit page; everywhere else queues.
