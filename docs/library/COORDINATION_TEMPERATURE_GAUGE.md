...
> Reference point: v5.38.4 · 2026-06-08
...
- **Sequence gap fallback (v5.79.42)**: if temperature jumps two zones in one bar (`temps[i-1] >= 55 && temps[i] < 45`, or the buy mirror `temps[i-1] < 45 && temps[i] >= 55`), Sequence `evaluate()` fires anyway. Same function drives on-chart markers and `backtestSignals`. Alternating cooldown unchanged. **No auto-execution** — signals stay signals. Reversion is `experimental: true` with a visible note (`#ruleExperimentalNote`) when picked; Sequence stays default. Do not revive gold-star spam. Kirk + Harmonia origin; CC/Opus registry; Celeste oversaw this layer. Hypha may check for cuts.
...
- **Luminos six-sprite architecture (v5.38.1)**: layer moved back inside `#chartWrap` (sprites were bouncing off gravity lines onto sub-charts when at chart-area level). Six sprites in three semantic pairs: **direction** (0, 1) carries buy/sell mood + gravity-spring energy; **alert** (2, 3) carries fresh-signal flare — bloom when temperature crossed 55 or 45 in the last ~5 bars, fade when stale; **intensity** (4, 5) carries raw `|temp-50|` magnitude independent of direction. Three new CSS variables on the layer: `--lum-alert`, `--lum-alert-color`, `--lum-intensity`, `--lum-intensity-color`. Opacity gated via `filter: opacity()` so the keyframe fade-in/fade-out stays alive. Four new keyframes (`tg-lum-3` through `tg-lum-6`) with distinct drift paths so the sprites don't visually clone each other. Trade-off accepted: signal coloring is quiet in tool-only mode (chartWrap is `display:none`); tool-only is a focused mode where ambient mood isn't wanted.
- **Luminos clarity recolor + bonus sprites (v5.38.4)**: Kirk's read on the v5.38.1 sprites — "I can see what is being done, but I don't think it is clear to everyone." Three color families that were stepping on each other (gold appeared in direction-neutral AND intensity-low AND alert-stale; lavender appeared in direction-neutral-complement AND adjacent to intensity) collapsed into three non-overlapping color identities so the dimensions can be named in one sentence:
    - **Direction → red / green / slate gray.** Both sprites in the pair stay in the same family (`c1` + lighter `c2` tint) so direction reads without color hunting. Neutral is slate gray, NOT gold, so no overlap.
    - **Alert → cyan family.** Lerp from soft cyan (#67e8f9) when stale → bright electric cyan (#06b6d4) when fresh. Alert color no longer carries buy/sell direction — the direction sprites already do that. Alert sprites only carry "something just fired."
    - **Intensity → purple family.** Lerp from pale lavender (#c4b5fd) at low intensity → vivid magenta (#c026d3) at high. Replaces the old gold→orange→red lerp.
  Kirk's sentence works now: "the red or green tells you direction, the purple tells you how intense the signal is, and the cyan tells you when something fresh has fired." Plus three **bonus sprites** (data-i 6/7/8) — one per pair — visible only when that pair's scalar exceeds 0.6 via `opacity(calc((var(--lum-X, 0) - 0.6) * 2.5))`. Low signal = 6 sprites visible. Loaded signal = up to 9. "More of them the bigger the signal" without noise at calm. Three more keyframes (`tg-lum-7/8/9`) for the bonus paths. Smoke locks color identity per pair, the bonus-sprite gating formula, and the 9-sprite count.
